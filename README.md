...
| clockIsRising       | Usually the clock pin of the rotary changes from low to high (the value is rising) when triggered. But there are some models with a falling behavoir. Try changing this value from `true` (default) to `false` if your rotary does not work with default settings | Boolean | `true` |
